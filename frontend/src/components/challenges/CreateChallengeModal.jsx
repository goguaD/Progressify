...
export default function CreateChallengeModal({ t, friends, onClose, onCreated }) {
  ...
  // Minimum date = tomorrow (date-only picker, no time)
  ...
  async function handleSend() {
    ...
    if (needsDeadline) {
      // deadline is a date string (YYYY-MM-DD); treat as start of next day (midnight)
      const dl = new Date(deadline)
      dl.setDate(dl.getDate() + 1) // end of the selected day = midnight next day
    ...
    // Convert date-only to midnight of the NEXT day (= 00:00 AM of deadline day boundary)
      ...
      <div className="modal-card modal-card--wide" onClick={(e) => e.stopPropagation()} role="dialog">
        ...
        {needsDeadline && (
          <>
            ...
            <input type="date" className="form-input" value={deadline}
              min={minDate} onChange={(e) => setDeadline(e.target.value)} />
          </>
        )}
